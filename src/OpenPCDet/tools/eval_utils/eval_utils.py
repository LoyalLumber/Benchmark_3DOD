...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None):
    ...
    p = open(f"/home/mj/YOLO/source/ssd/CIA-SSD/usg_data/pointpillar_mean_iou_0.75.txt", 'a')   
    p_data = f"{statistics.mean(FPS)} {statistics.mean(POW)} {statistics.mean(RAM)} {statistics.mean(CPU_usg)} {statistics.mean(GPU_usg)}\n" 
    p.write(p_data)
    f = open(f"/home/mj/YOLO/source/ssd/CIA-SSD/usg_data/pointpillar_inference_iou_0.75.txt", 'w')  
    for i in range(len(FPS)):
        A_data = f"{FPS[i]} {POW[i]} {RAM[i]} {GPU_usg[i]}\n" 
        f.write(A_data)
    f.close()
    g = open(f"/home/mj/YOLO/source/ssd/CIA-SSD/usg_data/pointpillar_cpuUsg_iou_0.75.txt", 'w')  
